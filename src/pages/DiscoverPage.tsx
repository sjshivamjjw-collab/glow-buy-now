        ...
        <div className="space-y-3">
          {filtered.map(c => {
            ...
            return (
              ...
              >
                {c.cover_url ? (
                  <div className="aspect-[21/9] bg-secondary overflow-hidden max-h-20">
                    <img src={c.cover_url} alt={c.name} className="w-full h-full object-cover" />
                  </div>
                ) : (
                  <div className="aspect-[21/9] max-h-20 bg-gradient-to-br from-primary/20 via-accent/10 to-secondary flex items-center justify-center">
                    <span className="text-xl font-extrabold text-primary/40">{c.name[0]}</span>
                  </div>
                )}
                <div className="p-2.5">
                  <h3 className="font-bold text-foreground text-sm mb-0.5">{c.name}</h3>
                  {c.description && <p className="text-xs text-muted-foreground line-clamp-1 mb-2">{c.description}</p>}
                  <div className="flex items-center justify-between text-[11px]">
                    <div className="flex items-center gap-1.5 text-muted-foreground">
                      {creator?.avatar_url ? (
                        <img src={creator.avatar_url} className="w-4 h-4 rounded-full object-cover" alt="" />
                      ) : (
