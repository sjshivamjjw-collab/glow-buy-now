...
import { Settings, LogOut, ChevronRight, Package, MapPin, CreditCard, Bell, HelpCircle, Store, ShieldCheck, ClipboardList, Check, X } from 'lucide-react';
import Footer from '@/components/Footer';

const ProfilePage = () => {
  const { role, userName, userAvatar, userId, isSeller, isAdmin, phone, setRole, logout, updateProfile, refreshRoles } = useAuth();
  ...
  useEffect(() => {
    if (!userId) return;
    // Pick up newly granted roles (e.g. just-approved seller) without re-login
    refreshRoles();
    ...
    supabase.from('orders').select('*', { count: 'exact', head: true }).eq(col, userId).then(({ count }) => {
  ...
  const handleSwitchRole = () => {
    const newRole = role === 'seller' ? 'shopper' : 'seller';
    setRole(newRole);
    navigate('/');
  };
  ...
  const menuItems = role === 'shopper'
    ? [
        { icon: Package, label: 'Order History', count: orderCount, path: '/orders' },
        { icon: MapPin, label: 'Saved Addresses', path: '/addresses' },
        { icon: CreditCard, label: 'Payment Methods', count: 0, path: '#' },
    ...
    : [
        ...
        { icon: Package, label: 'Orders', count: orderCount, path: '/orders' },
        { icon: Bell, label: 'Notifications', path: '/notifications' },
  ...
  return (
    <div className="min-h-screen bg-background max-w-lg mx-auto px-4 pt-14">
      ...
      {/* Switch role — only for approved sellers */}
      {isSeller && (
        <button
          onClick={handleSwitchRole}
          className="w-full flex items-center gap-3 px-5 py-4 rounded-2xl bg-primary/10 border border-primary/20 mb-6"
        >
          <Store className="w-5 h-5 text-primary" />
          <span className="flex-1 text-foreground font-semibold text-sm">
            Switch to {role === 'seller' ? 'Shopper' : 'Seller'} Mode
          </span>
          <ChevronRight className="w-4 h-4 text-primary" />
        </button>
      )}

      {/* Become a Seller — only for shoppers who aren't already sellers */}
      {role === 'shopper' && !isSeller && (
        <button
        ...
        {menuItems.map((item, i) => (
          ...
          >
            ...
            {'count' in item && item.count !== undefined && (
              <span className="px-2 py-0.5 rounded-full bg-secondary text-muted-foreground text-xs font-bold">{item.count}</span>
            )}
            <ChevronRight className="w-4 h-4 text-muted-foreground" />
          </button>
        ))}
